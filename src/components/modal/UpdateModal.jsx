...
import { getLastQuestionId, renumberSlateQuestionsSmart } from "../../utils";
...
const UpdateModal = ({
  ...
  const updateQuestions = async () => {
    ...
    const updateIndex = newQuestion.findIndex(item => item.id === selectedQuestion.id)
    ...
    setLoading(true);
    const requestBody = {
        questions: newUpdateQuestions.map(ques => {
          return {
            id: ques.id,
    ...
    try {
        const response = await apiClient.put(`api/v1/${type}/update/${id}/questions`, requestBody)
